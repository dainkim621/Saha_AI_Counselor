...
def extract_metadata(soup: BeautifulSoup):
    ...
    # 사하구청 상세 페이지의 정보 영역 타겟팅 (보통 .department, .charge, .contact 등)
    # 아래는 일반적인 관공서 사이트의 패턴입니다.
    info_area = soup.select_one(".infoBox, .board_view_info, .department")
    if info_area:
        text = info_area.get_text(" ", strip=True)
        # 정규표현식으로 날짜 추출 (예: 2024-04-05)
        date_match = re.search(r"(\d{4}-\d{2}-\d{2})", text)
        if date_match:
            metadata["date"] = date_match.group(1)
            
        # 담당부서나 작성자 추출 (텍스트 매칭 방식)
        if "부서" in text:
            # '부서 : 총무과' 같은 패턴에서 부서명만 추출 시도
            metadata["author"] = text.split("부서")[-1].split("|")[0].strip()
        ...
        while queue and len(visited) < MAX_PAGES:
            ...
            if not should_save(url):
                ...
                try:
                    ...
                    #새로 만든 메타데이터 추출 함수 호출
                    soup_for_meta = BeautifulSoup(html, "html.parser")
                    meta = extract_metadata(soup_for_meta)
                except Exception as e:
                    print(f"  파싱 실패: {e}")
