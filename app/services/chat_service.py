...
async def ask_saha_ai_stream(user_question: str, history: List[Dict[str, str]] = None):
            ...
            rewriter_messages = [
                {
                    ...
                    "content": (
                        ...
                        "이전 대화 기록을 철저히 분석하여, 사용자의 최신 질문에 대명사('그거', '이거')가 있거나 **주체(물품명 등)가 생략되어 있다면 이전 대화에서 언급된 핵심 대상(예: 소파, 책상 등)을 반드시 채워넣어 줘.** "
                        "설명은 빼고 오직 단 한 줄의 검색용 문장만 출력해. "
                        "입력받은 질문의 핵심 의도를 100% 보존하되, 사용자가 언급한 구체적인 물품명(예: 소파)을 임의로 빼거나 '대형폐기물' 같은 포괄적 단어로만 대체하여 의미를 모호하게 만들지 마. "
    ...
    messages = [
        ...
        "content": (
            ...
            "6. 정보의 한 단위(예: 하나의 물품/규격)는 한 줄에 작성하되, 참고 정보에 여러 개의 항목이나 규격이 존재할 경우 각각 줄바꿈을 하여 글머리 기호(-)로 모두 나열해. 💡\n"
